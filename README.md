...
__<ins>Если ip и порт уже использовались и записаны в сокет, советуется не нагружать SPI</ins>__  
...
uint8_t W5500_SendData(
    uint8_t            	  socket_num,   // номер сокета
    enum W5500_func_mode  func_mode,    // режим отправки
    const uint8_t*     	  dest_ip,      // IP адрес сервера
    uint16_t           	  dest_port,    // порт сервера
...
__<ins>ВАЖНО: Если сокет в режиме TCP, то перед каждой отправкой сокет открывается и закрывается, соединение с сервером так же 
разрывается и заново подключается.</ins>__
...
__<ins>Пример 1:</ins>__ вы передали буфер длинной в 20 байт, в результате функция вернула вам значение 15, 
следовательно в вашем буфере сейчас 15 байт информации и 5 незначащих.  
__<ins>Пример 2:</ins>__ вы передали буфер длинной в 20 байт, в результате функция вернула вам значение 47, 
